...
## 第零階段：環境調查與初始化 (若尚未完成)
如果你發現本 SOP 裡面的「最佳派工組合 (Menu)」中的「免費苦工」還是佔位符 (Placeholder)（例如寫著 `<本地模型>`），或者使用者明確要求重新調查本地模型，你必須：
1. 使用你的終端機執行能力（如 `run_command` 工具）去調查 Ollama 支援的模型：
   - 執行 `ollama list`。
2. 從調查結果中，挑選出一款最適合做苦工的**本地模型** (例如 qwen 系列、llama 系列等)。
3. 使用你的檔案編輯能力，**修改專案根目錄的 `CLAUDE.md`**，將你挑選出的本地模型結果，覆寫進「第一階段」的 ⚠️ 嚴格限制菜單中的「免費苦工」項目，取代掉舊的佔位符 `<本地模型>`。
4. 完成後，向使用者回報已更新本地菜單，然後再開始進行下方的第一階段。
...
2. 用 `<plan>` 標籤輸出任務拆解，標明依賴關係，並標明**每個子任務會動到哪些檔案**以及**適用的 Model**。
   **同一批平行的子任務，檔案清單不得重疊**（重疊就改成序列執行，否則後面必然 merge 衝突）。
   ...
   - 【敏捷主力】 `worker: agy_cli`, `model: gemini-3.7-flash-high` (適用：一般功能開發、UI、串接 API)
   - 【免費苦工】 `worker: ollama`, `model: <本地模型>` (適用：產生樣板、寫測資、翻譯、重複性工作)
   <plan>
   [子任務 1] 負責人: ollama | 模型: <本地模型> | 依賴: 無 | 檔案: src/models/* | 原因: 大量樣板生成
   [子任務 2] 負責人: agy_cli | 模型: gemini-3.7-flash-high | 依賴: 無 | 檔案: src/auth/* | 原因: 高階邏輯重構
...
3. **把拆解結果列給使用者看**（不可省略）。緊接在 `<plan>` 之後輸出這張表：
   ...
   | --- | --- | --- | --- | --- | --- | --- |
   | 1 | 產生 model 樣板 | ollama | <本地模型> | src/models/* | 無 | 待派工 |
   | 2 | 重構 auth 邏輯 | agy_cli | gemini-3.7-flash-high | src/auth/* | 無 | 待派工 |
...
2. **批次派發**：對每個子任務呼叫 `delegate_to_worker`，**必須**帶入上一步的 worktree 路徑。同時，**請務必傳入您在計畫表中決定的 `model` 參數**（例如 `model="claude-3-5-sonnet-20241022"` 或 `model="qwen2.5-coder:70b"`），以確保派工給具有相應能力的大腦。
   收齊所有 `job_id` 後才進下一步。
